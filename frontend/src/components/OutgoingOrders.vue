...
<div class="row">
    ...
    <div class="col-md-6">
        ...
        <div style="margin-top: 20px;" class="shadow-lg rounded p-3 table-responsive">
            ...
            <table class="table">
                <thead>
                    <tr> <th>Batch code</th> <th>Batch name</th> <th>Received</th> </tr>
                </thead>
                <tbody v-for="batch in batches_assigned">
                    <tr> <td> <i class="bi bi-qr-code icon"></i> ****** </td> <td> {{ batch.batch_name }} </td> 
    <td>

    <div class="form-check form-switch">
  <input class="form-check-input" type="checkbox" :disabled="batch.received=='Yes'" :checked="batch.received=='Yes'" @change="mark_batch_received(batch.batch_code)">
        ...
        <div class="p-3">
          ...
          <div style="margin-top:50px;" class="shadow rounded p-3">
           ...
           <div style="padding-top:30px;">
              <button class="btn theme-btn-2" @click="show_camera=true"> <i class="bi bi-camera"></i> Scan batches</button>
          </div>
...
export default{
    ...
    methods : {
        ...
        async outgoing_orders(){
            const res = await axios.get(this.$store.state.api_url+'api/outgoing-orders/'+this.business_id).then(function(response){
        ...
        async supplier_details(){
         ...
         async pay_order(){
            const res = await axios.post(this.$store.state.api_url+'api/pay-order',{
        ...
        async scan_batch(batch_code){
            const res = await axios.post(this.$store.state.api_url+'api/scan-batch',{
                batch_code : batch_code,
                order_id : this.order_id,
